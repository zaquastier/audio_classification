...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', type=str, required=True, help='Batch size to filter by.')
    parser.add_argument('--duration', type=str, required=True, help='Duration to filter by. Use "all" for all durations.')
    parser.add_argument('--number_of_epochs', type=str, required=True, help='Number of epochs to filter by. Use a comma-separated list for multiple values.')
    ...
    # Load JSON data
    with open('experiment_20231026.json', 'r') as f:
        data = json.load(f)
